...
// ── Jalankan query MySQL via CLI, return output ───────────────
static std::string mysql_query(const std::string &sql) {
    ...
    // Buat tmp file untuk password (hindari password di command line)
    std::string cnf = "/tmp/.sentinelx_my.cnf";
    {
        std::ofstream f(cnf);
        f << "[client]\nhost=" << host << "\nport=" << port
          << "\nuser=" << user << "\npassword=" << pass << "\n";
    ...
    std::string cmd = "mysql --defaults-file=" + cnf +
