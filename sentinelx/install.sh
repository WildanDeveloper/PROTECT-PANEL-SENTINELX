...
if [[ "$PKG_MGR" == "apt" ]]; then
    ...
    install_pkg "mysql-client"     "mysql" 2>/dev/null || install_pkg "mariadb-client" "mysql"
...
# ── 11. Block metadata cloud (anti-exploit) ──────────────────
echo ""
inf "Memblokir akses metadata cloud provider (169.254.169.254)..."
iptables -I FORWARD -d 169.254.169.254 -j DROP 2>/dev/null || true
iptables -I OUTPUT  -d 169.254.169.254 -j DROP 2>/dev/null || true

# Simpan biar persistent setelah reboot
if command -v iptables-save &>/dev/null; then
    mkdir -p /etc/iptables
    iptables-save > /etc/iptables/rules.v4 2>/dev/null && ok "iptables rules disimpan (persistent)" || warn "Gagal simpan iptables rules"
fi

# Fallback: cron reboot kalau iptables-persistent tidak ada
if ! grep -q "169.254.169.254" /etc/crontab 2>/dev/null; then
    echo "@reboot root iptables -I FORWARD -d 169.254.169.254 -j DROP && iptables -I OUTPUT -d 169.254.169.254 -j DROP" >> /etc/crontab
    ok "Cron reboot iptables ditambahkan"
fi
ok "Metadata cloud diblokir — user panel tidak bisa scan password VPS"
...
echo -e "  ${BLD}Firewall :${RST} 169.254.169.254 diblokir (metadata cloud)"
echo ""
