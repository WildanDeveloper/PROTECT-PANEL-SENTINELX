...
// ── Jalankan query MySQL/MariaDB, return output ──────────────
static std::string mysql_query(const std::string &sql) {
    if (!db_ready) return "";

    // Tulis tmp credentials file (hindari password di command line)
    std::string cnf = "/tmp/.sentinelx_trk.cnf";
    {
        std::ofstream fc(cnf);
        fc << "[client]\nhost=" << db_host << "\nport=" << db_port
           << "\nuser=" << db_user << "\npassword=" << db_pass << "\n";
    ...
    // Coba mysql dulu, fallback ke mariadb
    ...
    std::string cmd = cli + " --defaults-file=" + cnf +
        " --batch --skip-column-names " + shell_escape(db_name) +
        " -e " + shell_escape(sql) + " 2>/dev/null";

    std::array<char, 4096> buf;
    std::string result;
    std::shared_ptr<FILE> pipe(popen(cmd.c_str(), "r"), pclose);
    if (!pipe) { ::remove(cnf.c_str()); return ""; }
